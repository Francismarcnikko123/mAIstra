...
- **Save next to the tabs (2026-09-26, branch `feature/program-tabs-save`):**
  ...
  - **Tests** (`submissions-list.program-tabs.spec.ts`): 47 tests cover tabs, Save/shortcut/close behavior, conflicts, failures, pending edits to either Program 1 or an extra tab, restoring saved text, edits during saving, unverified OCR snapshots, partial saves, empty statuses, and the single template feedback location. Full web suite: **306/306** (18 files); application and spec TypeScript checks pass. Jayrald's `saveStatusMessage()` tests remain unchanged.

  - **Verification (2026-09-26):** `ng build` passes with only the existing `submissions-list.list.css` budget warning (11.63 kB against 11 kB). A local Chrome layout fixture passes all 15 combinations of saved/pending/conflict labels and 320/375/600/800/1280px widths. `npx playwright test` could not start because `@playwright/test` is absent from local dependencies; bundled Chromium is also absent. No cloud Save was performed.
