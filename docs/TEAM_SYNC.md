...
- [ ] (2026-09-27, from Nikko) **Code review finding for you and Nombrado:** two teachers on the same paper can lose a Program 3 without a conflict. `extraAnswers`/`savedExtras` are seeded once per paper, but a realtime refresh moves `codeBaseRevisions` to the newer revision, so the next save sends `p_replace_all = true` with a stale tab list that passes the revision check and deletes the other teacher's tab. Possible fix: refresh the tab list (or treat a newer revision as a conflict) when the page's programs changed. Details: `docs/changes/2026-09-27-code-review-fixes.md`, item 9.
...
- (2026-09-26) **Edit screen built and checked in the browser** (local, not yet pushed): *Edit* on the question page opens the form pre-filled, Save locked until validation passes, graded-papers warning before test-case changes, older questions can get a section and number. Web 309/309. Still to confirm on a device: two pages in one submit share one `batch_id`.
...
### Changed (affects others)
- (2026-09-27) **Code-review fixes** (`df06e0e`, `47851dc`; details in `docs/changes/2026-09-27-code-review-fixes.md`): phone retries resend only missing pages under one `batch_id`; recrop now auto-fixes and re-checks like a scan; the graded-papers warning also counts `submission_programs`; live uploads show their photo badge; relinking to an unsectioned question drops the old section name. **Nombrado:** your `saveStatusLabel()` now also reports unsaved Programs 2..n ("New changes need to be saved"), and still says "✓ Program 1 saved" in your preview mode. Please OK with the earlier two changes.
