...
- (2026-09-26) **Edit screen built** (local, not yet pushed): *Edit* / *Validate test cases* on the question page, Save locked until validation passes, graded-papers warning before test-case changes, older questions can get a section and number. Web 310/310; still to check in the browser. Still to confirm on a device: two pages in one submit share one `batch_id`.
