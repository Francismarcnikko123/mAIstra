...
- (2026-09-26, evening) **Review/Save work is complete; what happens next, in order:**
  ...
  5. **Nombrado (now, no data needed):** prepare the bond / yellow import: make `select_holdout.py` hold out whole new writers (writer-disjoint test set), add the `writer_id` column to both `labels.csv` files, and record `--verified-by` provenance (`NEXT_STEPS.md`). **When the papers arrive:** import, rebuild crops, retrain, compare on the same test set.
