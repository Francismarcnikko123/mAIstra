...
- **Next Nombrado tasks, in order:** (1) the Save-label follow-ups on branch `feature/review-save-followups` (Jayrald's requests in his TEAM_SYNC "Needs from others"); (2) the OCR training export reading every program from `submission_programs`, each block aligned to its own lines, so Jayrald can later drop the Program 1 mirror.
