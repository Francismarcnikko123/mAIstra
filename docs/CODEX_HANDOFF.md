...
## Current state — 2026-09-27 (supersedes the blocks below)

- **Branch:** `judge0-integration` for everyone (latest pushed work includes the Save-label follow-ups `4bf44f6`, the OCR export `b62bb0a`, and TEAM_SYNC handoffs `1c9c716`). **All older branches are kept only for diffs** (`feature/pre-extraction`, `feature/program-tabs`, `feature/program-tabs-save`, `feature/review-save-followups`, `feature/ocr-export-programs`, Nikko's `feature/question-linking*` / `feature/capture-quality-gate`, `main` / `develop`): never run their web app against the cloud, because they save by writing `submissions.verified_text` directly and leave Program 1 in `submission_programs` stale.
- **Open database gap (Jayrald's, in his TEAM_SYNC To do):** the cloud still allows that direct write. Proposed fix: the page copy follows Program 1 through a `SECURITY DEFINER` trigger, `save_submission_programs()` stops writing the copy, and `UPDATE (verified_text)` is revoked from the browser roles. Until then the OCR export warns on a mismatch.
- **Nombrado's web and export work is complete.** Next Nombrado step: the bond / yellow dataset (prep now, import when papers arrive); see `NEXT_STEPS.md`, 2026-09-27.
- **Checks on `judge0-integration` (2026-09-27):** web 309/309, OCR 250/250, `evaluate_cer` unchanged (clean_ws CER 0.099, WER 0.328, token accuracy 0.716), Playwright e2e 9/9 (`npm run e2e` in `maistra_web/`, fake backend, no cloud writes; installed locally with `npm install` + `npx playwright install chromium`).
- **Local-only noise, never commit:** `npm install` / `flutter` may rewrite `maistra_web/package-lock.json` or `maistra_mobile/pubspec.lock` for the local tool versions; restore them with `git checkout --`. `.claude/` is local preview config.
- **Mobile:** Nikko will remove the unused plugin folders in `maistra_mobile/packages/` (dead code). Until then, their `permission_handler` analyzer errors are local and harmless (`flutter pub get` inside them silences them); the app itself (`lib/`, `test/`) has no analyzer errors.
...
- **Checks on `judge0-integration` (2026-09-26; see 2026-09-27 above for current):** OCR 231/231, web 299/299, TypeScript clean, `ng build` passes (CSS budget warning on `submissions-list.list.css`, Jayrald's).
