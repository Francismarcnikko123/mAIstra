...
- A **Save** button at the right end of the program tab bar saves **every tab of the paper** in one update and keeps the teacher on Step 2. The unsaved dots clear and "✓ All programs saved" shows next to it. While the cloud database has no `answers` column, only Program 1 is stored: the label then says "✓ Program 1 saved", the extra tabs keep their dots, and the existing message explains why (fixed the same day; the first version claimed "All programs saved"). **Cmd/Ctrl+S** does the same on Step 2 (and no longer opens the browser's "save page" dialog there).
  - **Follow-up (2026-09-26, `feature/review-save-followups`):** `saveStatusLabel()` now shows save failures, the existing `saveStatusMessage()` conflict sentence, or “New changes need to be saved.” when any program differs from its saved state. `saveStatusTone()` uses red for errors/conflicts, amber for pending edits, and green for saved confirmations; empty status has no label. Pending edits take priority over the Program 1-only confirmation. The duplicate `.save-status` paragraph and its CSS are removed with Jayrald's OK. Long labels wrap; at narrow widths Save sits below the scrolling tabs. `EXTRA_PROGRAMS_UNSAVABLE` now names the missing `submission_programs` table (the `answers` column is gone). Save/conflict logic and `saveStatusMessage()` are unchanged.
