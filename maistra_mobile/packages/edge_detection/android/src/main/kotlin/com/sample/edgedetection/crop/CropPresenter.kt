...
class CropPresenter(
    ...
    fun onViewsReady(paperWidth: Int, paperHeight: Int) {
        val imgW = picture?.width() ?: 1080
        val imgH = picture?.height() ?: 1920

        // Compute the actual displayed rect inside the fitCenter ImageView
        val scale = minOf(paperWidth.toFloat() / imgW, paperHeight.toFloat() / imgH)
        val displayedW = (imgW * scale).toInt()
        val displayedH = (imgH * scale).toInt()
        val offsetX = (paperWidth - displayedW) / 2f
        val offsetY = (paperHeight - displayedH) / 2f

        iCropView.getPaperRect().onCorners2Crop(
            corners, picture?.size(), displayedW, displayedH, offsetX, offsetY
        )
    ...
    fun save() {
        ...
        // Save the original (pre-warp) photo alongside the cropped result so
        // the recrop flow can reload the full capture instead of the warped image.
        val saveTo = file.absolutePath
        val originalFile = File(
            if (saveTo.endsWith(".jpg", ignoreCase = true))
                saveTo.dropLast(4) + "_original.jpg"
            else
                "${saveTo}_original.jpg"
        )
        if (!originalFile.exists() && picture != null) {
            try {
                val origBitmap = Bitmap.createBitmap(picture.width(), picture.height(), Bitmap.Config.ARGB_8888)
                Utils.matToBitmap(picture, origBitmap)
                val outStream = java.io.FileOutputStream(originalFile)
                origBitmap.compress(Bitmap.CompressFormat.JPEG, 95, outStream)
                outStream.flush()
                outStream.close()
                origBitmap.recycle()
                Log.i(TAG, "Original photo saved: ${originalFile.absolutePath}")
            } catch (e: Exception) {
                Log.w(TAG, "Could not save original photo: ${e.message}")
            }
        }
