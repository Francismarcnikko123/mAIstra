...
class ScanActivity : BaseActivity(), IScanView.Proxy {
    ...
    override fun prepare() {
        ...
        val gestureDetector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            override fun onSingleTapUp(e: MotionEvent): Boolean {
                mPresenter.focusOnPoint(e.x, e.y, surfaceView.width, surfaceView.height)
                return true
            }
        })
        surfaceView.setOnTouchListener { _, event -> gestureDetector.onTouchEvent(event); true }
    ...
    override fun updateLightingFeedback(message: String?) {
        val banner = findViewById<TextView>(R.id.lighting_feedback)
        if (message == null) {
            banner.visibility = View.GONE
        } else {
            banner.text = message
            banner.setBackgroundColor(Color.parseColor("#CC7B0000"))
