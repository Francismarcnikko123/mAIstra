...
class PaperRectangle : View {
    ...
    init {
        rectPaint.color = Color.argb(128, 0, 120, 255)
        ...
        circlePaint.color = Color.argb(255, 0, 120, 255)
    ...
    fun onCorners2Crop(corners: Corners?, size: Size?, paperWidth: Int, paperHeight: Int,
                       displayOffsetX: Float = 0f, displayOffsetY: Float = 0f) {
        if (size == null) {
            return
        }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        rectPaint.color = Color.argb(255, 0, 120, 255)
        rectPaint.strokeWidth = 6F
        rectPaint.style = Paint.Style.STROKE
        canvas?.drawPath(path, rectPaint)

        rectPaint.color = Color.argb(60, 0, 120, 255)
    ...
    private fun resize() {
        tl.x = tl.x.div(ratioX) + offsetX
        tl.y = tl.y.div(ratioY) + offsetY
        tr.x = tr.x.div(ratioX) + offsetX
        tr.y = tr.y.div(ratioY) + offsetY
        br.x = br.x.div(ratioX) + offsetX
        br.y = br.y.div(ratioY) + offsetY
        bl.x = bl.x.div(ratioX) + offsetX
        bl.y = bl.y.div(ratioY) + offsetY
    }

    private fun reverseSize() {
        tl.x = (tl.x - offsetX).times(ratioX)
        tl.y = (tl.y - offsetY).times(ratioY)
        tr.x = (tr.x - offsetX).times(ratioX)
        tr.y = (tr.y - offsetY).times(ratioY)
        br.x = (br.x - offsetX).times(ratioX)
        br.y = (br.y - offsetY).times(ratioY)
        bl.x = (bl.x - offsetX).times(ratioX)
        bl.y = (bl.y - offsetY).times(ratioY)
    }
}
